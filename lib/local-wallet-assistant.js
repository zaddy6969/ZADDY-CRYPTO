...
function summarizeWallet(wallet) {
  if (!wallet?.connected) {
    return "No wallet is connected right now, so the dashboard can only comment on the visible Arc product state and readiness panels.";
  }
...
function summarizeActivity(activity, activityStatus, activityError) {
  if (activityStatus === "loading" || activityStatus === "refreshing") {
    return "The live Arc activity feed is still loading right now.";
  }

  if (activityStatus === "error") {
    return activityError
      ? `The live Arc activity feed is unavailable right now: ${activityError}`
      : "The live Arc activity feed is unavailable right now.";
  }

  if (!Array.isArray(activity) || activity.length === 0) {
    return "The dashboard does not currently list any recent onchain wallet activity.";
  }
...
export function generateLocalAssistantAnswer({ question, wallet, dashboard }) {
  ...
  if (
    ...
    return [
      ...
      walletSummary,
      "The wallet module handles connection and Arc USDC balance, the activity feed reflects live Arc events, and the assistant answers from the dashboard context visible right now."
    ].join(" ");
  }
